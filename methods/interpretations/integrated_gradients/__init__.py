...
class IntegratedGradientsWrapper():
    ...
    def __init__(self, model, tokenizer, class_names):
        ...
        self.interpreter = LayerIntegratedGradients(forward_with_softmax, model.bert.embeddings)
    ...
    def interpret(self, text, label, viz=True):
        ...
        attributions, delta = self.interpreter.attribute(inputs=(input_ids, token_type_ids, attention_mask),
                                            return_convergence_delta=True,
                                            additional_forward_args=(self.model),
                                            target=int(label))
        summarized_attributions= summarize_attributions(attributions)
                
        if viz:
            visualize(summarized_attributions, text, label, self.model, self.tokenizer)
            
        return summarized_attributions.numpy(), all_tokens
