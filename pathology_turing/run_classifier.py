...
from tnlr.modeling import TuringNLRv3ForSequenceClassification
...
def train_path_frozen(kwargs, train_dataset, eval_dataset, model, tokenizer, weights):
    """ Train the model """
    
    tb_writer = SummaryWriter()

    kwargs.train_batch_size = kwargs.per_gpu_train_batch_size * max(1, kwargs.n_gpu)
    ...
    eval_dataloader = DataLoader(
        eval_dataset, batch_size=kwargs.train_batch_size
    )
    ...
    # Freeze BERT
    if kwargs.freeze_layer:
        # We freeze here the embeddings of the model
        for param in model.bert.embeddings.parameters():
            param.requires_grad = False

        if kwargs.freeze_layer_count != -1:
            # if freeze_layer_count == -1, we only freeze the embedding layer
            # otherwise we freeze the first `freeze_layer_count` encoder layers
            print(len(model.bert.encoder.layer))
            for layer in model.bert.encoder.layer[:kwargs.freeze_layer_count]:
    ...
    warmup_steps = t_total * kwargs.warmup_ratio
    optimizer = AdamW(
        optimizer_grouped_parameters, lr=kwargs.learning_rate, eps=kwargs.adam_epsilon
    )
    ...
    logger.info("  Gradient Accumulation steps = %d", kwargs.gradient_accumulation_steps)
    logger.info("  Total optimization steps = %d", t_total)

    metric_for_best = kwargs.metric_for_choose_best_checkpoint
    ...
    train_iterator = trange(
        int(kwargs.num_train_epochs), desc="Epoch", disable=kwargs.local_rank not in [-1, 0]
    )
    set_seed(kwargs)  # Added here for reproductibility (even between python 2 and 3)
    
    # loss-epoch graph
    draw_loss, draw_epoch, draw_acc = [], [], []
    
    for _ in train_iterator:
        #epoch_iterator = tqdm(
        #    train_dataloader, desc="Iteration", disable=kwargs.local_rank not in [-1, 0]
        #)
        ...
        if kwargs.local_rank in [-1, 0]:
            logs = {}
            if (
                kwargs.local_rank == -1 and kwargs.evaluate_during_training
            ):  # Only evaluate when single GPU otherwise metrics may not average well
                results = evaluate(
                    eval_dataloader, kwargs, model, tokenizer, prefix="epoch-{}".format(_ + 1)
                )
                ...
                draw_acc.append(results[metric_for_best])
                
                if (
                    best_epoch is None
                    or best_performance[metric_for_best] < results[metric_for_best]
                ):
            ...
            # loss-epoch graph
            draw_loss.append(loss_scalar)
            draw_epoch.append(_)
            
            print(json.dumps({**logs, **{"step": global_step}}))

            # Save model checkpoint
            save_path = os.path.join(kwargs.output_dir, "save_output/")
            upload_path = os.path.join(kwargs.output_dir, "epoch-{}".format(_ + 1))
            ...
            torch.save(kwargs, os.path.join(save_path, "training_args.bin"))
            logger.info("Saving model checkpoint to %s", save_path)

            # only head node will save to mount
            if kwargs.local_rank in [-1, 0]:
                print(upload_path)
                ...
                try:
                    ...
                    shutil.copytree(save_path, upload_path)
    ...
    if best_epoch is not None:
        logger.info(
            " ***************** Best checkpoint: {}, choosed by {} *****************".format(
        ...
        if kwargs.local_rank in [-1, 0]:
            save_best_result(best_epoch, best_performance, kwargs.output_dir)
    
    sum_file = os.path.join(kwargs.output_dir, "summary.json")
    out_dict = {'draw_epoch':draw_epoch, 'draw_loss': draw_loss, 'draw_acc': draw_acc,
                'global_step':global_step, 'avg_loss': tr_loss / global_step}
    with open(sum_file, 'w') as fp:
        json.dump(out_dict, fp)
        
    return out_dict

def train_path(kwargs, train_dataset, eval_dataset, model, tokenizer, weights):
    """ Train the model """
...
def train(args, train_dataset, eval_dataset, model, tokenizer):
    ...
    eval_dataloader = DataLoader(
        eval_dataset, batch_size=args.train_batch_size
    )
    ...
    for _ in train_iterator:
        ...
        if args.local_rank in [-1, 0]:
            ...
            if (
                ...
                results = evaluate(
                    args, model, tokenizer, prefix="epoch-{}".format(_ + 1)
                )
                for key, value in results.items():
                    eval_key = "eval_{}".format(key)
                    logs[eval_key] = value
            ...
            # Save model checkpoint
            save_path = os.path.join(args.output_dir, "save_output/")
            upload_path = os.path.join(args.output_dir, "epoch-{}".format(_ + 1))
...
def main():
    ...
    parser.add_argument(
        "--metric_for_choose_best_checkpoint",
        type=str,
        default=None,
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            if metric_for_best is None:
                metric_for_best = list(result.keys())[0]
            if (
                best_epoch is None
                or best_performance[metric_for_best] < result[metric_for_best]
            ):
