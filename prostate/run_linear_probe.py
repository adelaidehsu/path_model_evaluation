...
from turing.pathology.run_classifier import MODEL_CLASSES, processors, train_path_frozen
...
elif args['model_type'] == 'random_bert':
    bert_path = 'bert-base-uncased'
    tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
...
for field in fields:
    ...
    args['output_dir'] = f"{base_dir}/output/frozen/{args['model_type']}_{args['run']}_{args['freeze_layer_count']}/{field}"
    args['cache_dir'] = f"{base_dir}/output/frozen/{args['model_type']}_{args['run']}_{args['freeze_layer_count']}/{field}"
    ...
    # Train model
    if args['model_type'] == 'random_bert':
        config = BertConfig.from_pretrained(bert_path)
        config.update({'num_labels': len(le_dict)})
    ...
    train_path_frozen(kwargs, train_dataset, val_dataset, model, tokenizer, class_weights)
